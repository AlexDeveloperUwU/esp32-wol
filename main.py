...
from config import MQTT_BROKER, MQTT_PORT, MQTT_CLIENT_ID, WOL_IP, WOL_MAC, LED_PIN
from utils import sync_time, get_dynamic_topic, verify_payload, send_magic_packet, ping_device
...
def get_system_stats():
    ...
    return '{"uptime":%d,"mem_free":%d,"mem_alloc":%d,"rssi":%d}' % (uptime_s, mem_free, mem_alloc, rssi)
...
def sub_callback(topic, msg):
    ...
    try:
        ...
        print(f"[RX] Payload: {payload_str}")
        
        cmd, ts = verify_payload(payload_str)
        ...
        if cmd == "WAKE":
            ...
            client.publish(resp_topic, resp)
            flash_signal(2)
            ...
            resp_topic = current_topic + "/response"
            print(f"[TX] {stats}")
            client.publish(resp_topic, stats)
            flash_signal(2)
            
        elif cmd is None:
            print("[!!] Invalid Signature/Timestamp")
